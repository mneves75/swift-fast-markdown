...
struct ListView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: CGFloat(style.blockSpacing)) {
            // Note: Array(enumerated()) allocates on each body evaluation.
            // Acceptable here because: (1) lists are typically small (5-50 items),
            // (2) body only evaluates when list content changes, (3) index is
            // needed for ordered list markers, and alternatives (firstIndex or
            // storing ordinal in IR) would be O(n²) or require invasive changes.
